...
def select_export_case(
    ...
    *,
    ...
    """Select an export case through country, stage, buyer, and case filters."""
    ...
    buyers = sorted({_text(case['buyer'], '바이어 미입력') for case in stage_cases})
    preferred_buyer = _text(saved_case['buyer'], '바이어 미입력') if saved_case in stage_cases else None
    _set_valid_state(buyer_key, buyers, preferred_buyer)
    selected_buyer = buyer_col.selectbox('바이어', buyers, key=buyer_key)

    buyer_cases = [
        case for case in stage_cases
