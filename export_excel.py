...
def build_packing_export(case_id: int) -> bytes:
    ...
    ws.merge_cells('A1:K2'); ws['A1']='EXPORT PACKING DETAIL'; ws['A1'].font=Font(size=18,bold=True); ws['A1'].alignment=Alignment(horizontal='center',vertical='center')
    ...
    ws['F7']='배송 상세'; ws['G7']=detail; ws.merge_cells('G7:K7'); ws['F7'].fill=PatternFill('solid',fgColor=LIGHT); ws['F7'].font=Font(bold=True)
    headers=['박스번호','사업장','로케이션','제품명','LOT','유통기한','수량','가로(cm)','세로(cm)','높이(cm)','무게(kg)']
    r=10
    ...
    for box_no, group in groups.items():
        first=current
        for item in group:
            values=[box_no,item['business_unit'],item['location'],item['product_name'],item['lot_no'],item['expiry_date'],item['requested_qty'],item['length_cm'],item['width_cm'],item['height_cm'],item['weight_kg']]
            for c,v in enumerate(values,1):
                cell=ws.cell(current,c,v); cell.border=border(); cell.alignment=Alignment(vertical='center',wrap_text=True)
            current+=1
        last=current-1
        if last>first:
            for col in [1,8,9,10,11]: ws.merge_cells(start_row=first,start_column=col,end_row=last,end_column=col)
        for col in [1,8,9,10,11]: ws.cell(first,col).alignment=Alignment(horizontal='center',vertical='center')
    widths=[11,14,14,32,16,14,10,11,11,11,11]
