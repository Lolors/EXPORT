...
def packing_rows(case_id: int):
    return db.rows(
        '''SELECT s.box_no, s.business_unit, s.location, s.product_name, s.lot_no,
...
def progress_state(ordered_qty: float, linked_qty: float) -> tuple[str, str]:
    if ordered_qty > 0 and linked_qty >= ordered_qty:
        return 'done', '확보 완료'
    if linked_qty > 0:
...
case_id = options[st.selectbox('수출 건 선택', list(options), key='delivery_document_case')]
...
with edit_tab:
    ...
    if submitted:
        db.execute("UPDATE export_cases SET domestic_method=?,tracking_no=?,driver_name=?,driver_phone=?,actual_ship_date=?,stage='국내배송',status='완료',updated_at=? WHERE id=?", (method, tracking, driver, phone, str(actual_date), db.now_text(), case_id))
    ...
    for order in orders:
        state_class, state_label = progress_state(float(order['quantity'] or 0), float(order['linked_qty'] or 0))
        icon = {'done':'🟢','partial':'🟡','pending':'🔴'}[state_class]
