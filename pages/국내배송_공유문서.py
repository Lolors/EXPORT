...
def render_document(case, rows) -> None:
    ...
    document = f'''
    <style>
    .export-document {{max-width:1180px; margin:8px auto 28px; background:#fff; border:1px solid #d8dee8;
        border-radius:14px; box-shadow:0 12px 34px rgba(30,45,70,.08); overflow:hidden; color:#172033;}}
    .doc-header {{padding:34px 40px 28px; background:linear-gradient(135deg,#173b5f,#245d88); color:#fff;
    ...
    .status-pill {{display:inline-block; margin-top:10px; padding:6px 12px; border-radius:999px;
    ...
    .info-grid {{display:grid; grid-template-columns:repeat(4,1fr); border:1px solid #dce3eb; border-radius:9px;
    ...
    .note-box {{margin-top:22px; border:1px solid #dce3eb; border-left:4px solid #294f71; border-radius:7px;
        padding:14px 16px; background:#fafbfd; font-size:13px;}}
    .note-title {{font-size:11px; color:#778493; font-weight:800; margin-bottom:5px;}}
    .doc-footer {{display:flex; justify-content:space-between; margin-top:28px; padding-top:14px;
        border-top:1px solid #e2e7ed; color:#8a94a1; font-size:10px;}}
    @media print {{
        [data-testid="stSidebar"], [data-testid="stHeader"], .stTabs, .stButton, .stDownloadButton {{display:none !important;}}
        .block-container {{padding:0 !important; max-width:none !important;}}
        .export-document {{box-shadow:none; border:none; margin:0; max-width:none;}}
    ...
    @media(max-width:800px) {{
        .doc-header {{padding:25px 22px; flex-direction:column;}}
        .doc-number {{text-align:left;}}
        .doc-body {{padding:22px;}}
        .info-grid {{grid-template-columns:1fr 1fr;}}
        .info-cell {{border-bottom:1px solid #e5eaf0;}}
...
with document_tab:
    ...
    with toolbar2:
        st.caption('공유할 때는 브라우저의 인쇄 기능(Ctrl+P)에서 PDF로 저장하면 문서 영역만 출력됩니다.')
