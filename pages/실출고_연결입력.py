...
st.set_page_config(page_title='실출고 연결 입력', page_icon='📦', layout='wide')
db.init_db()
...
st.title('실출고 연결 입력')
st.caption('주문품목별로 실제 확보한 제품의 사업장, 로케이션, 제품명, 제조번호, 유통기한과 수량을 연결합니다.')
...
selected_label = st.selectbox('수출 건 선택', list(options), key='linked_shipment_case')
...
if unlinked_count:
    st.warning(f'기존 실출고 데이터 중 주문품목에 연결되지 않은 행이 {unlinked_count}개 있습니다. 아래에서 필요한 주문품목에 다시 입력해 주세요.')
...
for order in orders:
    ...
    with st.expander(
        f"{icon} {order['product_name']} · 주문 {fmt_number(order_qty)} {unit} · 확보 {fmt_number(linked_qty)} {unit} · {state}",
        expanded=linked_qty < order_qty,
    ):
        st.caption('한 주문품목에 실제 제품이나 제조번호가 여러 개면 행을 추가해서 각각 입력하세요.')
        ...
        st.info(
            f'{preview_icon} 입력 합계 {fmt_number(preview_qty)} / 주문 {fmt_number(order_qty)} {unit} · {preview_state}'
        )
